...
const debug = require('debug')('flatm8:translator'),
      nconf = require('nconf');
...
class Translator {
  ...
  translate (text, languageCode) {
    debug('going to translate %s', text.substring(0, 50));
  ...
  translateKey (name, args, languageCode) {
    ...
    if (namespace && !key) {
      debug(`Missing key in translation token "${name}"`);
    ...
    return Promise.all(argsToTranslate).then((result) => {
      ...
      if (!translated) {
        debug(`Missing translation "${name}"`);
   ...
   */
  static getLanguage () {
    return nconf.get('language') || 'de_DE';
  }
  ...
  loadLanguage (languageCode) {
    ...
    if (this.languages.hasOwnProperty(languageCode)) {
      debug(`language ${languageCode} has already been loaded`);
    ...
    return new Promise((resolve, reject) => {
      try {
        this.languages[ languageCode ] = require(`../../public/translations/${languageCode}.json`);
